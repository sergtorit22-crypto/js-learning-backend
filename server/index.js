...
const pool = require("./db"); //  підключення до PostgreSQL
...
// Мідлвари
app.use(cors());
app.use(express.json()); // Обов'язково для обробки даних із форм React
...
app.post("/register", async (req, res) => {
  try {
    ...
    // Додавання в базу (зберігаємо hashedPassword замість password)
...
app.post("/login", async (req, res) => {
  try {
    ...
    // Порівнюємо введений текст (password) з хешем з бази (user.rows[0].password)
...
app.get("/lessons", async (req, res) => {
  try {
    // 1. Отримуємо всі уроки (поля id, title, material, code)
    ...
    for (let lesson of lessons) {
      // 2. Отримуємо всі тести для конкретного уроку за його id
      ...
      for (let test of tests) {
        // 3. Отримуємо ВСІ варіанти відповідей для цього тесту з таблиці options
        ...
        if (test.type === "choice") {
          // Якщо тип 'choice' — передаємо масив усіх доступних варіантів для кнопок
          test.options = allOptions.map((opt) => opt.option_text);
        } else {
          // Якщо тип 'text' — масив варіантів залишаємо порожнім, бо кнопок не буде
          test.options = [];
        }

        // 4. ЗНАХОДИМО ПРАВИЛЬНУ ВІДПОВІДЬ (універсально для обох типів)
        // Шукаємо той рядок, де is_correct === true
        const correctRow = allOptions.find((opt) => opt.is_correct === true);

        // Передаємо текст правильної відповіді у фронтенд для порівняння в TestSystem
        test.correct = correctRow ? correctRow.option_text : "";
      }

      // Додаємо масив тестів до об'єкта уроку
      lesson.tests = tests;
    }

    // Відправляємо фінальний масив уроків на фронтенд
    res.json(lessons);
  } catch (err) {
    console.error("Помилка сервера:", err.message);
    res.status(500).send("Server error while fetching lessons and tests");
  }
});
...
app.post("/results", async (req, res) => {
  try {
    ...
    // Запит із використанням ON CONFLICT та виправленням часового поясу
    // Використовуємо 'Europe/Kyiv' для точного часу в Україні
    const query = `
    ...
    // Повертаємо оновлений або створений запис
    res.status(200).json(updatedResult.rows[0]);
  } catch (err) {
    console.error("Помилка при збереженні результатів тесту:", err.message);
    res.status(500).json({ error: "Помилка сервера при записі в БД" });
  }
});

// Маршрут для отримання результатів конкретного користувача
app.get("/lesson_results", async (req, res) => {
  ...
  try {
    // SQL-запит до вашої таблиці за схемою
    // Вибираємо всі результати для конкретного юзера
    const queryText = `
    ...
    // Повертаємо масив об'єктів клієнту
    res.json(results.rows);
  } catch (err) {
    console.error("Помилка при виконанні запиту до БД:", err);
    res
      .status(500)
      .json({ error: "Помилка сервера при отриманні результатів" });
  }
});
// --- ДОДАВАННЯ УРОКУ (ТРАНЗАКЦІЯ) ---
app.post("/add-lesson", async (req, res) => {
  ...
  try {
    ...
    for (const q of questions) {
      ...
      // 3. Вставляємо варіанти залежно від типу
      if (q.type === "choice") {
        ...
        // Для текстового питання створюємо один запис у таблиці options
        await client.query(
          "INSERT INTO options (test_id, option_text, is_correct) VALUES ($1, $2, $3)",
...
// 1. Отримання даних ОДНОГО уроку для редагування
...
app.put("/edit-lesson/:id", async (req, res) => {
  ...
  try {
    ...
    // 1. Оновлюємо основні дані уроку
    await client.query(
      "UPDATE lessons SET title = $1, material = $2, code = $3 WHERE id = $4",
      [title, material, code, id],
    );

    // 2. Видаляємо старі тести (каскадно видаляться і options, якщо в БД стоїть ON DELETE CASCADE)
    // Якщо CASCADE не налаштовано, спочатку видаліть options через JOIN або subquery
    await client.query("DELETE FROM tests WHERE lesson_id = $1", [id]);

    // 3. Додаємо оновлені питання (як у роуті add-lesson)
    for (const q of questions) {
      const testRes = await client.query(
        "INSERT INTO tests (lesson_id, question, type, created_by, created_at) VALUES ($1, $2, $3, $4, NOW() AT TIME ZONE 'Europe/Kyiv') RETURNING id",
